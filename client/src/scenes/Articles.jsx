  ...
  return (
    <div>
        <div className="flex flex-col md:flex-row justify-between min-[948px]:px-32 md:px-16 items-center gap-5 md:gap-0 pt-12 md:pt-0">
          ...
          className="md:pb-10 text-green text-4xl md:text-6xl lg:text-7xl">{posts[0].title}</Link>
          <p dangerouslySetInnerHTML={{ __html: posts[0].content.slice(0,35) + "..."}} className='ml-auto mr-0 md:w-1/2 md:text-3xl lg:text-4xl break-words hidden md:block'/>
          </div>
        </div>
        <ScrollPosts 
          text="See more "
          subtext="posts."
        />
        <ScrollCategoriesPosts1 
          category="Women In Stem"
        />
        <ScrollCategoriesPosts2 
          category="Tutorials"
        />
    </div>
  )
